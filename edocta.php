...
if ($accion == "Verificar") {	
	...
	$sql="select * from confarm where factura = '$lafactura' and (forma = 'Farmacia') ";
...
if ($accion == 'Buscar')  {
	...
	$temp = "";
	echo "<form enctype='multipart/form-data' action='edocta.php?accion=Verificar' name='form1' method='post' onsubmit='return valsoc(form1)'>";
	pantalla_prestamo($result,$cedula);
	$elstatus=$_SESSION['elstatus'];

		echo '<fieldset><legend>Resumen para Farmacia</legend>';
		$conta = $_GET['conta'];
		if (!$_GET['conta']) 
			$conta = 1;
...
// 		bucle de listado
		...
		echo "<td class='centro'><a href='edocta.php?ano=".trim($row['ano'])."&accion=ConsultaAno&cedula=".trim($row['cedula'])."&'><img src='imagenes/page_wizard.gif' width='16' height='16' border='0' title='Ver Detalle' alt='Ver Detalle' /></a></td>";
...
if ($accion == "ConsultaAno") {	
	...
	echo '<fieldset><legend>Resumen para Farmacia Año <strong>'.$ano.'</strong></legend>';
...
// 		bucle de listado
		...
		while($row=mysql_fetch_assoc($rs)) {
			...
			echo "<td class='centro'><a href='edocta.php?factura=".trim($row['factura'])."&accion=VerFactura'><img src='imagenes/page_wizard.gif' width='16' height='16' border='0' title='Ver Factura' alt='Ver Factura' /></a></td>";
			echo "<td class='centro'>";
...
if (!$accion) {
	echo "<form action='edocta.php?accion=Buscar' name='form1' method='post'>";
    echo '  C&eacute;dula ';
...
if ($mostrarregresar==1) { // ($accion == "Buscar") or ($accion == "Ver") or ($accion="EscogePrestamo")) {
	echo '<form enctype="multipart/form-data" name="formdepie" method="post" action="edocta.php?accion=Buscar">';

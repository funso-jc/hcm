...
if ($accion == 'Buscar')  {
	...
	$temp = "";
	echo "<form enctype='multipart/form-data' action='reghcm.php?accion=Verificar' name='form1' method='post' onsubmit='return valreg(form1)'>";
...
// 		bucle de listado
		...
		echo "<td class='centro'><a href='reghcm.php?ano=".trim($row['ano'])."&accion=ConsultaAno&cedula=".trim($row['cedula'])."&'><img src='imagenes/page_wizard.gif' width='16' height='16' border='0' title='Ver Detalle' alt='Ver Detalle' /></a></td>";
...
// 		bucle de listado
		...
		while($row=mysql_fetch_assoc($rs)) {
			...
			echo "<td class='centro'><a href='reghcm.php?factura=".trim($row['factura'])."&accion=VerFactura'><img src='imagenes/page_wizard.gif' width='16' height='16' border='0' title='Ver Factura' alt='Ver Factura' /></a></td>";
			echo "<td class='centro'>";
...
if (!$accion) {
	echo "<form action='reghcm.php?accion=Buscar' name='form1' method='post'>";
    echo '  C&eacute;dula ';
...
if ($mostrarregresar==1) { // ($accion == "Buscar") or ($accion == "Ver") or ($accion="EscogePrestamo")) {
	echo '<form enctype="multipart/form-data" name="formdepie" method="post" action="reghcm.php?accion=Buscar">';
